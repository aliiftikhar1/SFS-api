...
from Beats_Management.Serializers import InstrumentSerializer, SubInstrumentSerializer, \
    CreateSubInstrumentSerializer, InstrumentDropdownSerializer
from Beats_Management.models import BeatInstrument
from Utilities import extract_error_messages
from Utilities.Permissions import AdminPermissions, SupplierPermissions


class BeatInstrumentsDropdownView(APIView):
    ...
    def get(request):
        ...
        instrument = BeatInstrument.objects.prefetch_related("sub_instrument").all()
...
class BeatInstrumentsView(APIView):
    ...
    def delete(request):
        ...
        instrument = BeatInstrument.objects.filter(pk=instrument_id, name=instrument_name).first()
        if instrument:
            try:
                instrument.delete()
...
class BeatSubInstrumentsView(APIView):
    permission_classes = [IsAuthenticated, AdminPermissions]
    ...
    def get(request):
        ...
        instrument_id = request.GET.get("instrument_id")
        instrument_name = request.GET.get("instrument_name")
        instrument = BeatInstrument.objects.prefetch_related("sub_instrument").filter(pk=instrument_id,
